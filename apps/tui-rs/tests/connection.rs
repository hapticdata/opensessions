use opensessions_sidebar::client::{decode_server_message, validate_hello, EXPECTED_PROTOCOL_VERSION};
use opensessions_sidebar::generated::protocol::{ProtocolHello, ServerMessage};

#[test]
fn accepts_matching_protocol_hello() {
    let hello = ServerMessage::Hello(ProtocolHello { protocol: EXPECTED_PROTOCOL_VERSION, server_version: "0.2.0-alpha.5".into() });
    assert!(validate_hello(&hello).is_ok());
}

#[test]
fn rejects_mismatched_protocol_hello() {
    let hello = ServerMessage::Hello(ProtocolHello { protocol: EXPECTED_PROTOCOL_VERSION + 1, server_version: "future".into() });
...
fn decodes_text_server_message_payload() {
    let msg = decode_server_message(br#"{"type":"hello","protocol":1,"serverVersion":"0.2.0-alpha.5"}"#).unwrap();
